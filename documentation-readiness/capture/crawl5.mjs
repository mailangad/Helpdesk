// Crawler v5: waits for pages to finish loading. opens each module by its real URL, then clicks every item in the
...
const SHOTS = path.join(OUT, 'screens5');
...
const settle = async () => {
  ...
  // Wait until spinners, skeletons and "Loading" text are gone (max ~20s).
  for (let i = 0; i < 40; i++) {
    const busy = await page.evaluate(() => {
      const sel = '[class*="spinner" i],[class*="loading" i],[class*="skeleton" i],[class*="loader" i],[role=progressbar],mat-spinner,mat-progress-spinner';
      const visible = [...document.querySelectorAll(sel)].some((e) => {
        const r = e.getBoundingClientRect();
        return r.width > 0 && r.height > 0 && getComputedStyle(e).visibility !== 'hidden' && getComputedStyle(e).display !== 'none';
      });
      return visible || /\bLoading\b/i.test(document.body.innerText.slice(0, 5000));
    }).catch(() => false);
    if (!busy) break;
...
async function record(name) {
  ...
  fs.writeFileSync(path.join(OUT, 'app-inventory-v5.json'), JSON.stringify(inventory, null, 2));
...
async function leftMenuItems() {
  return page.evaluate(() => {
    ...
    for (const e of document.querySelectorAll('*')) {
      const r = e.getBoundingClientRect();
      if (r.left < 0 || r.right > 205 || r.top < 330 || r.bottom > 860 || r.height < 20 || r.height > 60) continue;
...
async function clickLeft(text) {
  const handle = await page.evaluateHandle((t) => {
    return [...document.querySelectorAll('*')].reverse().find((e) => {
      const r = e.getBoundingClientRect();
      return r.right < 205 && r.top > 330 && r.height >= 20 && r.height <= 60 &&
        getComputedStyle(e).cursor === 'pointer' && (e.innerText || '').trim().split('\n').pop() === t;
    });
  }, text);
  const el = handle.asElement();
  if (!el) throw new Error('menu item not found');
  await el.click();
}
